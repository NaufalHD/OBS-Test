...
-figured out that it will involves 3 type of data structure. 1st list is for the initial flat data (each node of the list is an object of Category with its own attributes), 2nd is a hashmap. Iterate through the flat list, and add each of them into the new map, with id attribute uses as the key, and the whole object will put into the value of the map. Why use hashmap? because of how it able to search directly on a data (using on key) without having to loop/iterate the whole list just for a single data (gonna be useful later on the parent-child assigning process)<br>
...
-With that consideration in mind, I build a function to let the user know that there's duplicate on this record and the system keep the first data registered and dumped the duplicate (while on the process of transforming flat list into hashmap)<br>
